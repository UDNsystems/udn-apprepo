...
#output_width = 41 too ducking slow
...
for frame_number in range(twerk_frame_count):
	f = open(f"sussified_{frame_number}.png","rb")
	js.postMessage(**{'event': 'custom:ffmpeg-save-file', 'filename': f'sussified_{frame_number}.png', 'data': pyodide.to_js(f.read())})
	f.close()

js.postMessage(**{'event': 'custom:run-ffmpeg', 'args': pyodide.to_js(['-f','image2','-i','sussified_%d.png','-filter_complex','[0:v] scale=sws_dither=none:,split [a][b];[a] palettegen=max_colors=255:stats_mode=single [p];[b][p] paletteuse=dither=none','-r','20','-y','-hide_banner','-loglevel','error','sussified.gif'])})
